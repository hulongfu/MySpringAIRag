...
✅ **流式输出（SSE）** ⭐ NEW
- 答案逐字显示（打字机效果），减少用户等待焦虑
- 后端使用 ChatClient.stream() 实现真正的流式调用
- 前端使用 Fetch API + ReadableStream 接收SSE流
...
└──────┬──────────────┘
       ↓
┌──────────────────────────┐
│ ParallelVectorRetrieval   │ ← 混合检索：3路并行
│ Service                   │
│                           │
│  ├─ Variant 1: 原始查询   │ → 向量检索（JVector）
│  ├─ Variant 2: 查询重写   │ → 向量检索（JVector）
│  └─ Keyword Search        │ → 关键词检索（IK+LIKE）
└──────┬───────────────────┘
       ↓ (并行执行)
┌─────────────────────┐
...
└──────┬──────────────┘
       ↓
┌─────────────────────┐
│ DocumentRepository   │ ← 关键词检索（LIKE）
│ .keywordSearch()     │    返回Top-200候选
└──────┬──────────────┘
       ↓
┌─────────────────────┐
│ RRF Fusion           │ ← 倒数排名融合3路结果
│ (Reciprocal Rank     │
│  Fusion)             │
└──────┬──────────────┘
       ↓
...
### 2. ParallelVectorRetrievalService（混合检索服务）⭐ UPDATED

**职责**：执行混合检索（2个向量变体 + 1个关键词检索）

**工作流程**：
1. **生成查询变体**：
   - 变体1：原始查询
   - 变体2：重写查询（QueryRewriteService优化）
2. **并行执行3路检索**：
   - 向量检索1：使用原始查询生成向量，JVector搜索
   - 向量检索2：使用重写查询生成向量，JVector搜索
   - 关键词检索：IK分词 + LIKE模糊匹配（DocumentRepository）
3. **RRF融合**：倒数排名融合3路结果
4. **返回排序后的文档列表**

**关键特性**：
- ✅ **3路并行**：同时执行2个向量检索和1个关键词检索
- ✅ **RRF算法**：`RRF_Score(doc) = Σ (1 / (K + rank_i))`，其中 K=60
- ✅ **兼顾语义与精确**：向量检索捕捉语义相似度，关键词检索保证精确匹配
- ✅ **提升召回率**：多路检索互补，减少漏检
...
2. **停用词过滤**：去除常见停用词（包括功能动词、助动词等）
3. **核心关键词提取**：保留名词、动词等实词
4. **去重处理**：移除重复关键词
5. **单字过滤**：过滤长度<2的词汇

**应用场景**：
- 用于ParallelVectorRetrievalService中的关键词检索
- 用于RagService中的关键词分数调整
- 提升检索结果的相关性

### 3.5 DocumentRepository（关键词检索实现）⭐ NEW

**职责**：提供基于关键词的数据库检索功能

**关键方法**：
- `keywordSearch(query, limit)`：关键词搜索
  - 使用IK分词器提取查询中的关键词
  - 构建LIKE模糊匹配SQL查询
  - 返回匹配的文档ID列表
  - 支持多关键词OR查询
...
WHERE LOWER(content) LIKE LOWER('%keyword1%') 
   OR LOWER(content) LIKE LOWER('%keyword2%') 
LIMIT ?
```
...
- `answerQuestion()`：回答问题（同步版本）
  ...
  - RRF融合
  - **关键词分数调整** ⭐ NEW
- `answerQuestionStream()`：**流式回答问题** ⭐ NEW
  - 复用检索逻辑
  - 使用 ChatClient.stream() 流式调用LLM
  - 通过 SSE 逐token推送答案
...
app:
  top-k: 5               # 最终返回数量
  use-parallel-retrieval: true  # ✅ 启用混合检索（2向量+1关键词）
```

**混合检索说明**：
- **2个向量变体**：原始查询 + 重写查询，捕捉语义相似度
- **1个关键词检索**：IK分词 + LIKE匹配，保证精确匹配
- **RRF融合**：3路结果倒数排名融合，兼顾召回率和精确率
...
**注意**：当前使用混合检索（2向量+1关键词），不再需要配置parallel-query-variants。
...
- [x] ✅ **流式输出**（SSE逐字显示，打字机效果）⭐ NEW
- [x] ✅ **空文本过滤与null向量防护**（三层防护机制）⭐ NEW
- [x] ✅ **删除冗余代码**：移除未使用的RRF融合方法和调试日志
